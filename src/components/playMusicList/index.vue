<template>
  <div class="playmusiclist">
    <van-sticky offset-top="30vh">
      <div class="playmusiclist__head">
        <p>播放列表</p>
        <svg
          t="1666109993550"
          class="icon"
          ...
          width="200"
          height="200"
          @click.stop="store.delAllMusicList"
        >
          <path
    ...
    <div class="playmusiclist__item">
      <div v-for="(item, index) in store.musicList" :key="index">
        <Item
          :musicName="item.name"
          :singerName="item.al.name"
          :index="index"
          :class="
            index == data.showPlayIcon
              ? 'playmusiclist__show-click'
              : 'playmusiclist__show'
          "
          :showPlayIcon="data.showPlayIcon"
          @click.stop="isShowPlayIcon(index)"
        />
      </div>
    </div>
  </div>
...
import Item from "@/components/playMusicList/item.vue";
...
const data = reactive({
  showPlayIcon: 0,
//   musicLst: store.musicList
});

//播放列表切换歌曲
const isShowPlayIcon = (index: number) => {
  data.showPlayIcon = index;
  store.index = index;
};

//监听非播放列表切换歌曲
watch(() => {store.index},() => {
    data.showPlayIcon = store.index;
},{
    deep:true,
});
...
.playmusiclist {
  ...
  background-color: rgba(168, 235, 18, 0.5);
  &__head {
    width: 100%;
    height: 0.8rem;
